...
void ArrayOfSumForEachColm(int Arra[3], int Matrix[3][3], int Rows, int Colms)
{
	for (int i = 0; i < 3; i++)
	{
		Arra[i] = SumOfEachColm(Matrix, Rows, i);
	}
}
void PrintSum(int Arra[3],int Matrix[3][3], int Rows, int Colms)
{
	for (int i = 0; i < 3; i++)
	{
		printf("Colm %d Sum = %d\n", i + 1, Arra[i]);
...
int main()
{
	...
	cout << "\nSum of each Colm : \n";
	ArrayOfSumForEachColm(Arra, Matrix, 3, 3);
	PrintSum(Arra,Matrix, 3, 3);
